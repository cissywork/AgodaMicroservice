...
# Connect to your email (example with Gmail)
...
# Search for emails from the current week
now = datetime.now()
start_of_week = now - timedelta(days=365)
search_since = start_of_week.strftime("%d-%b-%Y")
print(f"[DEBUG] Searching emails since: {search_since}")

status, messages = imap.search(None, f'(SINCE {search_since} FROM "no-reply@agoda.com" SUBJECT "Agoda Booking ID" SUBJECT "Hotel Country: Canada Check-in")')
email_ids = messages[0].split()
print(f"[DEBUG] Found {len(email_ids)} email(s)")
...
for num in reversed(email_ids):
    ...
    for response in msg:
        ...
        email_date = msg["Date"]
        try:
            last_updated_dt = datetime.strptime(email_date[:31], "%a, %d %b %Y %H:%M:%S %z").replace(tzinfo=None)
...
print("✅ reservations.json file has been saved.")